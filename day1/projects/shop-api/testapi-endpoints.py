...
def update_product_price_by_id(product_id, price):
    r = requests.put(f"{endpoint_url}/products", params={"product_id": product_id, "price": price})
    print(f"Updating product {product_id} with price {price}...")
    print(r.text)

def delete_product_by_id(product_id):
    r = requests.delete(f"{endpoint_url}/products", params={"product_id":product_id})
...
update_product_price_by_id(0, 1500)
print()
