...
class LibraryDB:
    ...
    def create_backup_database(self):
        self.backup_db = sqlite3.connect(self.backup_file, check_same_thread=False)

    def create_database(self):
        self.db = create_engine(f"sqlite+pysqlite:///{self.file}")
        self.create_table()

    def restore_backup(self):
        self.create_backup_database()
        raw_db = self.db.raw_connection()
        self.backup_db.backup(raw_db.driver_connection)
        raw_db.close()
    ...
    def get_user_by_token(self, token, session):
        decoded_token = decode_access_token(token)
        if not decoded_token:
            raise InvalidTokenError
        user = self.get_user_by_name(session, decoded_token["sub"])
        return user

    def get_user(self, token, session, user_id):
        user = self.get_user_by_token(token, session)
        if not (user.role in advanced_roles):
            return False
        user_result = session.get(User, user_id)
        if not user_result:
            return None
        response = self.user_result_to_account_response(user_result)
        return response

    def get_users(self, token, session):
        user = self.get_user_by_token(token, session)
        if not (user.role in advanced_roles):
            return False
    ...
    def modify_user_role(self, token, session, user_id, new_role):
        user = self.get_user_by_token(token, session)
        if user.role != "admin":
            return False
    ...
    def get_user_by_name(self, session, username):
        user_query = select(User).where(
            User.username == username
        )
        try:
            result = session.execute(user_query).scalar_one()
        except sqlalchemy.exc.NoResultFound:
            return None
        return result

    def add_user(self, session, creation_details: UserCreationRequest):
        user = self.get_user_by_name(session, creation_details.username)
        if user:
            return False
        new_user = User(
    ...
    def filter_books(self, token, session, author=None, title=None, year=None,
    ...
    def get_books(self, token, session, all_books=False):
        user = self.get_user_by_token(token, session)
        if not all_books:
    ...
    def get_book_by_id(self, token, session, book_id):
        user = self.get_user_by_token(token, session)
        if user.role == "user":
            query = select(Books).where(
                and_(
    ...
    def get_book_count(self, token, session):
        user = self.get_user_by_token(token, session)
    ...
    def get_book_stats(self, token, session):
        user = self.get_user_by_token(token, session)
        query = select(
    ...
    def get_latest_book(self, token, session):
        user = self.get_user_by_token(token, session)
    ...
    def get_available_books(self, token, session):
        available_books = self.filter_books(token, session, available=True)
        return available_books

    def get_unavailable_books(self, token, session):
        unavailable_books = self.filter_books(token, session, available=False)
        return unavailable_books

    def add_book(self, token, session, book_details: BookCreationRequest):
        user = self.get_user_by_token(token, session)
        if user.role == "user":
            new_book = Books(
                user_id=user.id,
    ...
    def remove_book(self, token, session, book_id):
        user = self.get_user_by_token(token, session)
        if user.role == "user":
            query = select(Books).where(
                and_(
        ...
        try:
            book = session.execute(query).scalar_one()
        except sqlalchemy.exc.NoResultFound:
            book = None
        if book:
    ...
    def update_book(self, token, session, book_id, book_details: BookUpdateRequest):
        user = self.get_user_by_token(token, session)
        updated = False
    ...
    def restore_book(self, token, session, book_id):
        user = self.get_user_by_token(token, session)
